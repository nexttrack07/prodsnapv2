[Skip to main content](https://clerk.com/docs/guides/sessions/session-tokens#main)

# Session tokens

1. [Default claims](https://clerk.com/docs/guides/sessions/session-tokens#default-claims)
2. [Size limitations](https://clerk.com/docs/guides/sessions/session-tokens#size-limitations)
1. [Example](https://clerk.com/docs/guides/sessions/session-tokens#example-2)
3. [Validate session tokens](https://clerk.com/docs/guides/sessions/session-tokens#validate-session-tokens)

Copy as markdownMarkdownCopy as markdown

[Open inOpen in ChatGPTOpenAI](https://chatgpt.com/?q=Read+https%3A%2F%2Fclerk.com%2Fdocs%2Fguides%2Fsessions%2Fsession-tokens.md&hints=search)
...
Read more about Clerk session tokens and how they work in [the guide on how Clerk works](https://clerk.com/docs/guides/how-clerk-works/overview).

## [Default claims](https://clerk.com/docs/guides/sessions/session-tokens\#default-claims)
...
| `fea` | features | A list of enabled Features and their scope. The scope can either be `o` for Organization-level Features if there is an Active Organization⁠, or `u` for user-level Features or if there is no active Organization. | `o:dashboard,o:impersonation` |
| `sts` | session status | The status of the current session | `pending` |

### [Organization claim](https://clerk.com/docs/guides/sessions/session-tokens\#organization-claim)

The **`o` claim**, or Organization claim, is only included if the user is part of an [Organization](https://clerk.com/docs/guides/organizations/overview) and that Organization is active⁠. Its value is an object that contains the following properties:
...
| `fpm` | feature-permission map | feature-permission map | The mapping of Features with Permissions. [Learn how this value is constructed](https://clerk.com/docs/guides/sessions/session-tokens#decode-o-fpm-manually). | `3,2` |

Warning

The Organization claims above are intentionally designed to be as compact as possible to keep JWT tokens small.
As a result, they can be difficult to decode manually. We strongly recommend using one of our SDKs that support API version [2025-04-10](https://clerk.com/docs/guides/development/upgrading/versioning#2025-04-10) to handle decoding reliably.

#### [Decode `o.fpm` manually](https://clerk.com/docs/guides/sessions/session-tokens\#decode-o-fpm-manually)
...
#### [Example](https://clerk.com/docs/guides/sessions/session-tokens\#example)
...
### [Actor claim](https://clerk.com/docs/guides/sessions/session-tokens\#actor-claim)

The **`act` (actor) claim** is only included if the user is [impersonating](https://clerk.com/docs/guides/users/impersonation) another user. It's value is an object that contains the following properties:
...
If you would like to add custom claims to your session token, you can [customize it](https://clerk.com/docs/guides/sessions/customize-session-tokens).

You can also create custom tokens using a [JWT template](https://clerk.com/docs/guides/sessions/jwt-templates).

## [Size limitations](https://clerk.com/docs/guides/sessions/session-tokens\#size-limitations)
...
A session token with the [default session claims](https://clerk.com/docs/guides/sessions/session-tokens#default-claims) won't run into this issue, as this configuration produces a cookie significantly smaller than 4kb. However, this limitation becomes relevant when implementing a [custom session token](https://clerk.com/docs/guides/sessions/customize-session-tokens). In this case, it's recommended to move particularly large claims out of the token and fetch these using a separate API call from your backend.
...
If you add any of these custom claims in your token, use caution to ensure the stored data doesn't exceed the size limit. It's highly recommended to [store the extra data in your own database](https://clerk.com/docs/guides/development/webhooks/syncing#storing-extra-user-data) instead of storing it in metadata in the session token. If this isn't an option, you can [move particularly large claims like these out of the token](https://clerk.com/docs/guides/sessions/session-tokens#example) and fetch them using a separate API call from your backend.

Note

If your application encounters this issue, the Clerk Dashboard will display a warning: **"Some users are exceeding cookie size limits"**. To resolve this, update your [custom session token](https://clerk.com/docs/guides/sessions/customize-session-tokens).

### [Example](https://clerk.com/docs/guides/sessions/session-tokens\#example-2)

It's recommended to keep the total size of custom claims in the session token under 1.2KB. The following example shows how to move particularly large claims out of the session token and fetch them using a separate API call from your backend. The limitations of this approach are that if you make this call to Clerk's Backend API frequently, you risk hitting [rate limits](https://clerk.com/docs/guides/how-clerk-works/system-limits) and it's also slower than making a database query. We highly recommend [storing the extra data in your own database](https://clerk.com/docs/guides/development/webhooks/syncing#storing-extra-user-data) instead of storing it in metadata in the session token.
...
Then, when you need to access the other metadata fields, you can fetch them using a separate API call from your backend. The following example uses the [`getUser()`](https://clerk.com/docs/reference/backend/user/get-user) method to access the current user's [Backend `User` object](https://clerk.com/docs/reference/backend/types/backend-user), which includes the `publicMetadata` field.
...
## [Validate session tokens](https://clerk.com/docs/guides/sessions/session-tokens\#validate-session-tokens)
...
To learn how to manually verify a session token, refer to the [dedicated guide](https://clerk.com/docs/guides/sessions/manual-jwt-verification).
...
[GitHubEdit on GitHub](https://github.com/clerk/clerk-docs/edit/main/docs/guides/sessions/session-tokens.mdx)
